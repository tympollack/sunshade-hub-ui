    ...
    >
      ...
      <div className="relative z-10 w-full max-w-lg p-6 sm:p-8 rounded-3xl bg-slate-900/90 backdrop-blur-xl border border-indigo-500/30 shadow-2xl space-y-6 animate-in fade-in zoom-in-95 duration-300 mx-auto">
        ...
        <div className="text-center space-y-3">
          <div className="mx-auto w-16 h-16 rounded-2xl flex items-center justify-center border border-indigo-400/30 shadow-xl"
            style={{
              background: 'linear-gradient(135deg, rgba(79,70,229,0.25) 0%, rgba(16,185,129,0.2) 100%)',
            }}
          >
          ...
          <div>
            <div className="inline-flex items-center gap-1.5 px-3 py-0.5 rounded-full text-[11px] font-bold font-mono tracking-wider text-indigo-300 bg-indigo-950/80 border border-indigo-700/60 uppercase">
              <Sparkles className="w-3 h-3 text-indigo-400" />
              <span>Account Onboarding</span>
            </div>
            <h1 className="text-2xl font-extrabold text-slate-100 tracking-tight mt-1.5">
          ...
          <div className="flex items-center justify-between gap-2 bg-slate-950/60 p-2 rounded-xl border border-slate-800">
            <div
              className={`flex-1 text-center py-1.5 rounded-lg text-xs font-semibold font-mono transition-colors ${
                step === 1 ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-500 hover:text-slate-300'
              }`}
            >
              1. Code & Email
            </div>
            <div
              className={`flex-1 text-center py-1.5 rounded-lg text-xs font-semibold font-mono transition-colors ${
                step === 2 ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-500 hover:text-slate-300'
              }`}
            >
              2. Account Setup
            </div>
        ...
        {successData ? (
          <div className="text-center py-6 space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-xs text-slate-300 max-w-sm mx-auto">
                Welcome to SunShade, <strong className="text-emerald-400">{successData.email}</strong>! Your authentication profile is active.
              </p>
            </div>

            <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-2">
              <p className="text-xs text-slate-400">Redirecting to Dashboard in</p>
              <div className="font-mono text-3xl font-extrabold text-indigo-400">{redirectCount}s</div>
            </div>

            <button
              onClick={() => (window.location.href = successData.redirectUrl)}
              className="w-full flex items-center justify-center gap-2 py-3 px-5 rounded-xl text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-500 transition-colors shadow-lg"
            >
          ...
          <form onSubmit={handleNextStep} className="space-y-4">
            ...
            <button
              type="submit"
              className="w-full flex items-center justify-center gap-2 py-3 px-5 rounded-xl text-xs font-bold text-white shadow-lg transition-all hover:scale-[1.01] active:scale-95 border border-indigo-500/40 mt-4"
              style={{
          ...
          <form onSubmit={handleFinalSubmit} className="space-y-4">
            ...
            <div className="flex gap-2 pt-2">
              ...
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 py-3 px-5 rounded-xl text-xs font-bold text-white shadow-lg transition-all hover:scale-[1.01] active:scale-95 border border-indigo-500/40 disabled:opacity-50"
                style={{
        ...
        {/* Footer brand note */}
        <div className="pt-2 text-center text-[11px] font-mono text-slate-500 flex items-center justify-center gap-1.5 border-t border-slate-800/80">
