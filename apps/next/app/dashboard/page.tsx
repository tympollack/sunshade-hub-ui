...
export default function DashboardPage() {
  ...
  const toggleChessAchievement = async (achievementId: string, isUnlocked: boolean) => {
    ...
    if (isUnlocked) {
      await supabase.schema('chess').from('user_achievements')
        .delete()
      ...
      await supabase.schema('chess').from('user_achievements')
        .insert({ user_id: session.user.id, achievement_id: achievementId });
  ...
  const toggleHubAchievement = async (achievementId: string, isUnlocked: boolean) => {
    ...
    if (isUnlocked) {
      await supabase.from('user_hub_achievements')
        .delete()
        .eq('user_id', session.user.id)
        .eq('achievement_id', achievementId);
  ...
  // Ensure Tailwind styles for scrollbar are active
  useEffect(() => {
          ...
          <header className="h-16 border-b border-zinc-200 dark:border-zinc-800/60 bg-white/80 dark:bg-[#161616]/80 backdrop-blur-md flex items-center justify-between px-4 sm:px-8 shrink-0 transition-colors duration-200">
            ...
            <div className="flex items-center gap-4">
              <span className="text-sm font-medium text-zinc-500 dark:text-zinc-400 hidden lg:block">
                Welcome back, {session?.user?.email || 'test@sunshade.local'}
              </span>
              
              {mounted && (
                <button 
                  onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}
                  className="p-2 text-zinc-500 hover:text-zinc-800 hover:bg-zinc-100 dark:text-zinc-400 dark:hover:text-zinc-200 dark:hover:bg-zinc-800 rounded-full transition-colors"
                >
              ...
              <div className="flex items-center gap-3 pl-4 border-l border-zinc-200 dark:border-zinc-800 transition-colors duration-200">
                <div className="text-right hidden sm:block">
                  ...
                  <p className="text-xs text-orange-600 dark:text-orange-400 font-mono">14,250 HT</p>
                </div>
                <div className="w-9 h-9 rounded-full bg-gradient-to-br from-orange-500 to-orange-700 dark:from-orange-600 dark:to-orange-800 flex items-center justify-center shadow-lg shadow-orange-500/20 border border-orange-400/30">
                  <span className="font-bold text-sm text-white">{session?.user?.email?.charAt(0).toUpperCase() || 'T'}</span>
                </div>
              </div>
            </div>
          </header>
              ...
              {activeView === 'Overview' && (
                <>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 bg-white dark:bg-[#161616] border border-zinc-200 dark:border-zinc-800/60 rounded-xl p-6 shadow-sm dark:shadow-none transition-colors duration-200">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
                        <StatBox label="Matches Played" value="142" />
                        <StatBox label="Win Rate" value="64%" />
                        <StatBox label="Local Currency" value="2,400 CP" />
                        <StatBox label="Achievements" value="8/50" />
                      </div>

                      <div className="space-y-3">
                        <h4 className="text-sm font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-3">Recent Matches</h4>
                        <MatchRow result="Victory" opponent="GrandmasterG" format="Chess960" moves={42} />
                        <MatchRow result="Defeat" opponent="NoobSlayer99" format="Standard" moves={18} />
                        <MatchRow result="Victory" opponent="ChessBot_Alpha" format="Standard" moves={56} />
                      </div>
                    </div>
                    ...
                    <div className="bg-white dark:bg-[#161616] border border-zinc-200 dark:border-zinc-800/60 rounded-xl p-6 shadow-sm dark:shadow-none transition-colors duration-200">
                      ...
                      <div className="space-y-4 relative before:absolute before:inset-0 before:ml-2 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-zinc-200 dark:before:via-zinc-800 before:to-transparent">
                        <ActivityItem 
                          ...
                          desc="First Blood (+50 CP)" 
                          time="2h ago" 
                          color="bg-orange-500" 
                        />
                        <ActivityItem 
                          title="Tokens Claimed" 
                          desc="Governance Airdrop (+500 HT)" 
                          time="5h ago" 
                          color="bg-blue-500" 
                        />
                        <ActivityItem 
                          title="Node Connected" 
                          desc="SunShade-US-East-1" 
                          time="1d ago" 
                  ...
                  <div className="bg-white dark:bg-[#161616] border border-zinc-200 dark:border-zinc-800/60 rounded-xl p-6 shadow-sm dark:shadow-none mt-6 transition-colors duration-200">
                    ...
                    {isLoading ? <p className="text-zinc-500 dark:text-zinc-400">Loading achievements...</p> : (
                      <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                          <thead>
                            <tr className="border-b border-zinc-200 dark:border-zinc-800 transition-colors duration-200">
                          ...
                          <tbody>
                            {hubAchievements?.map((ach) => (
                              <tr key={ach.id} className={`border-b border-zinc-100 dark:border-zinc-800/50 transition-all duration-200 ${userHubUnlocks[ach.id] ? 'opacity-100' : 'opacity-60'}`}>
                                <td className="p-3">
                                  <button 
                                    onClick={() => toggleHubAchievement(ach.id, !!userHubUnlocks[ach.id])}
                                    className={`px-3 py-1.5 rounded text-sm font-medium transition-colors ${userHubUnlocks[ach.id] ? 'bg-orange-100 dark:bg-orange-500/20 text-orange-600 dark:text-orange-400 hover:bg-orange-200 dark:hover:bg-orange-500/30' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400 hover:bg-zinc-200 dark:hover:bg-zinc-700'}`}
                                  >
                          ...
                          <tbody>
                            {chessAchievements?.map((ach) => (
                              <tr key={ach.id} className={`border-b border-zinc-100 dark:border-zinc-800/50 transition-all duration-200 ${userChessUnlocks[ach.id] ? 'opacity-100' : 'opacity-60'}`}>
                                <td className="p-3">
                                  <button 
                                    onClick={() => toggleChessAchievement(ach.id, !!userChessUnlocks[ach.id])}
                                    className={`px-3 py-1.5 rounded text-sm font-medium transition-colors ${userChessUnlocks[ach.id] ? 'bg-blue-100 dark:bg-blue-500/20 text-blue-600 dark:text-blue-400 hover:bg-blue-200 dark:hover:bg-blue-500/30' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400 hover:bg-zinc-200 dark:hover:bg-zinc-700'}`}
                                  >
...
function GameLibraryCard({ game, onSelect }: { game: any, onSelect: () => void }) {
  return (
    ...
    >
      ...
      <h3 className="text-xs sm:text-sm font-semibold text-zinc-800 dark:text-zinc-200 text-center leading-tight">
        {game.title}
      </h3>
    </button>
  );
  ...
  return (
    <button 
      ...
      className={`flex flex-col items-center gap-1 p-2 min-w-[64px] transition-colors ${
        active ? 'text-orange-600 dark:text-orange-400' : 'text-zinc-500 dark:text-zinc-400 hover:text-zinc-800 dark:hover:text-zinc-200'
      }`}
    >
      {icon}
  ...
  return (
    <a 
      ...
      className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-200 ${
        active 
          ? 'bg-orange-50 dark:bg-orange-600/10 text-orange-600 dark:text-orange-400 border border-orange-200 dark:border-orange-500/20' 
          : 'text-zinc-500 dark:text-zinc-400 hover:text-zinc-800 dark:hover:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-800/50 border border-transparent'
      }`}
    >
      {icon}
...
function MatchRow({ result, opponent, format, moves }: { result: 'Victory' | 'Defeat', opponent: string, format: string, moves: number }) {
  ...
  return (
    <div className="flex items-center justify-between p-3 rounded-lg bg-white dark:bg-zinc-900/40 border border-zinc-200 dark:border-zinc-800/40 hover:bg-zinc-50 dark:hover:bg-zinc-800/60 transition-colors duration-200 shadow-sm dark:shadow-none">
      ...
      <button className="text-xs font-medium text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white px-3 py-1.5 rounded bg-zinc-100 dark:bg-zinc-800/50 hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors">
        Review
      </button>
    </div>
  );
